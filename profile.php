...
$purchases_sql = "SELECT * FROM Purchases WHERE user_id='$user_id'";
...
<head>
   ...
   .background {
    ...
    width: 100%;
    height: 100%;
    background-image: url('11.png'); /* Укажите правильный путь к изображению */
    ...
    <ul class="list-group mt-3">
        ...
        if ($workouts_result->num_rows > 0) {
            while ($workout = $workouts_result->fetch_assoc()) {
                ...
                echo '</div>';
                echo '<form method="post" action="delete_workout.php" style="margin: 0;">';
                echo '<input type="hidden" name="workout_id" value="' . $workout['id'] . '">';
...
>
    ...
    <p class="text-center">Рассчитайте оптимальные пропорции БЖУ для ваших целей.</p>
    <button type="button" class="btn btn-info btn-block mt-4" data-toggle="modal" data-target="#bjuCalculatorModal">Открыть калькулятор</button>
</div>
            ...
            if ($subscriptions_result->num_rows > 0) {
                while ($subscription = $subscriptions_result->fetch_assoc()) {
                    ...
                    if ($subscription['status'] == 'pending') {
                        echo '<button class="btn btn-success btn-sm" data-toggle="modal" data-target="#paySubscriptionModal" data-subscription-id="' . htmlspecialchars($subscription['id']) . '">Оплатить</button>';
                    }
                    echo '<form method="post" action="delete_subscription.php" style="display:inline;">';
                    echo '<input type="hidden" name="subscription_id" value="' . htmlspecialchars($subscription['id']) . '">';
                    echo '<button type="submit" class="btn btn-danger btn-sm">Удалить</button>';
                    echo '</form>';
                    echo '</li>';
                }
    ...
    <div class="card purchases-card">
        <h2 class="text-center mb-4">Мои покупки</h2>
        <ul class="list-group">
            <?php
            if ($purchases_result->num_rows > 0) {
                while ($purchase = $purchases_result->fetch_assoc()) {
                    echo '<li class="list-group-item">';
                    echo '<strong>Товар:</strong> ' . htmlspecialchars($purchase['item']) . '<br>';
                    echo '<strong>Цена:</strong> ' . htmlspecialchars($purchase['price']) . ' руб.<br>';
                    echo '<strong>Статус:</strong> ' . htmlspecialchars($purchase['status']);
                    if ($purchase['status'] == 'pending') {
                        echo '<button class="btn btn-success btn-sm" data-toggle="modal" data-target="#payPurchaseModal" data-purchase-id="' . htmlspecialchars($purchase['id']) . '">Оплатить</button>';
                    }
                    echo '</li>';
                }
            } else {
                echo '<li class="list-group-item">Нет покупок</li>';
            }
            ?>
        </ul>
    </div>
...
<!-- Модальное окно для оплаты покупок -->
<div class="modal fade" id="payPurchaseModal" tabindex="-1" role="dialog" aria-labelledby="payPurchaseModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="payPurchaseModalLabel">Оплатить покупку</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="payPurchaseForm" method="post" action="pay_purchase.php">
...
<!-- Модальное окно для оплаты абонементов -->
<div class="modal fade" id="paySubscriptionModal" tabindex="-1" role="dialog" aria-labelledby="paySubscriptionModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="paySubscriptionModalLabel">Оплатить абонемент</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="paySubscriptionForm" method="post" action="pay_subscription.php">
...
<script>
    ...
    $('#payPurchaseModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget);
        var purchaseId = button.data('purchase-id');
        var modal = $(this);
        modal.find('#purchase_id').val(purchaseId);
    });

    $('#paySubscriptionModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget);
